...
#     drifting request shape corrupts or rejects a write. Only one of those is worth
#     waking up for.
#   * STRUCTURE ONLY - property names, types, enum values, required flags. Summaries,
#     descriptions and examples are excluded, because a detector that cries wolf over
...
def _deref(spec: dict, node: dict, seen: frozenset) -> tuple[dict, frozenset]:
    """Follow a $ref. Returns the node unchanged if it is not one, or if following
    would revisit a ref already on this path (the spec has cyclic DTOs)."""
    ref = node.get("$ref")
    if not ref or not ref.startswith("#/"):
        return node, seen
    if ref in seen:
        return {"type": f"<cycle {ref.rsplit('/', 1)[-1]}>"}, seen
    target = spec
    for part in ref[2:].split("/"):
        target = (target or {}).get(part) or {}
    return target, seen | {ref}


def schema_shape(spec: dict, node, seen: frozenset = frozenset(), depth: int = 0):
    """Reduce a JSON Schema to a canonical, prose-free description of its structure."""
    if not isinstance(node, dict):
        return "?"
    node, seen = _deref(spec, node, seen)
    if depth >= SCHEMA_MAX_DEPTH:
        return "<max-depth>"

    for combiner in ("allOf", "oneOf", "anyOf"):
        if combiner in node:
            return {combiner: [schema_shape(spec, s, seen, depth + 1)
                               for s in node[combiner]]}

    typ = node.get("type")
    if typ == "array":
        return {"array": schema_shape(spec, node.get("items") or {}, seen, depth + 1)}
    props = node.get("properties")
    if props or typ == "object":
        return {
            "required": sorted(node.get("required") or []),
            "props": {k: schema_shape(spec, v, seen, depth + 1)
                      for k, v in sorted((props or {}).items())},
        }
    leaf = typ or "any"
    if node.get("enum"):
        leaf = f"{leaf}[{','.join(sorted(map(str, node['enum'])))}]"
    if node.get("format"):
        leaf = f"{leaf}:{node['format']}"
    return leaf
...
def write_schema_hashes(spec: dict, org: list[dict]) -> dict[str, str]:
...
def write_fingerprint(spec, org, other, sha, date):
    routes = sorted(f"{o['method']} {o['path']}" for o in org + other)
    schemas = write_schema_hashes(spec, org)
    OUT_FINGERPRINT.write_text(json.dumps({
        ...
        "write_schema_count": len(schemas),
        "routes": routes,
...
def check_drift() -> int:
    ...
    if added or removed:
        print(f"\nROUTE DRIFT: +{len(added)} / -{len(removed)}")
        for r in added:
            print("  + ", r)
        for r in removed:
            print("  - ", r)
...
def main() -> int:
    ...
    write_fingerprint(spec, org, other, sha, date)
    write_map(org, entities, owners, sha, date, total, reachable)

    n_schemas = len(write_schema_hashes(spec, org))
    ...
    print(f"  fingerprint {len(org) + len(other)} routes + {n_schemas} write schemas "
          f"-> {OUT_FINGERPRINT.relative_to(REPO)}")
