---
name: wxcc-bulk
description: Use when asked to create, update, or delete MANY Webex Contact Center objects of one type in a single operation - "bulk update these queues", "create 20 entry points from this list", "bulk-create these global variables", "delete all these queues", "change the service level on every X queue at once". Mutating - requires cjp:config_write and explicit confirmation. Verified for contact-service-queue, entry-point, auxiliary-code, dial-number, outdial-ani, and cad-variable (global variables) - and which of create/update/delete each supports differs. Other entities are refused until probed.
---
...
| `outdial-ani` | ✅ | ❌ | ✅ | the API has no bulk update (id-bearing item → 400 "cannot have an id"); use `wxcc_update` or the entry tools |
...
- **queues** and **auxiliary-code** have a native partial-patch endpoint — only your fields
  are sent.
- **entry-point**, **cad-variable**, **dial-number** have no partial endpoint, so the tool
  **read-modify-writes**: it fetches each object and merges your fields into a full-object
  save. Same call for you either way.
- **outdial-ani** has **no** bulk update at all — the tool refuses it (use `wxcc_update` or
  the `wxcc_*_entry` tools).
...
| Routes/ops are NOT uniform | Queues use `contact-service-queue/v2/bulk` (create/delete) + `contact-service-queue/bulk` (PATCH update); every other verified entity is on `{entity}/bulk`. Some have a PATCH partial route (queues, aux-code), some don't. Some lack an op entirely (outdial-ani no update, dial-number no delete). `skill/v2/bulk` 404s. Only verified (entity, op) pairs are accepted — see the capability matrix. |
...
and an entry-point's dial-number reference); and the two unsupported ops (`outdial-ani`
update, `dial-number` delete) are refused by the tool, not attempted. Request envelope is
